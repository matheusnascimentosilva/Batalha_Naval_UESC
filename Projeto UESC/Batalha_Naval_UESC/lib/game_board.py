    ...
    def draw_life( grid, ships):
        ...
        for x in range( CONSTANTS.SIZE ):
            for y in range( CONSTANTS.SIZE ):
                if grid[y][x] == "S":#Porta-Aviões - Salobrinho
                    ships[0] = False
                if grid[y][x] == "M":#Cruzador - Morro Pernambuco
                    ships[1] = False
                if grid[y][x] == "B":#Fragata - Basílio
                    ships[2] = False
                if grid[y][x] == "G":#Corveta - Gabriela
                    ships[3] = False
                if grid[y][x] == "T":#Submarino Teotônio Vilela
                    ships[4] = False
                if grid[y][x] == "R":#Submarino Rio Cachoeira
                    ships[5] = False
                if grid[y][x] == "F":#Hidroaviao - Feira Malhado
                    ships[5] = False
